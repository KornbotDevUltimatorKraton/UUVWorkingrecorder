...
# Register
  ...
  AngleDegY = 0 
  AngleDegZ = 0 
  cap = cv2.VideoCapture(0)  # Video camera capture for the underwater drone 
  fourcc = cv2.cv.CV_FOURCC(*'XVID') # cv.videowriter   
  time = datetime.datetime.now()
  videoOut = cv2.VideoWriter("UUV"+str(time)+".avi",fourcc,20.0,(640,480)) # Video output size 
...
#   print("gyroskop_xout: ", ("%5d" % gyroskop_xout), " skaliert: ", (gyroskop_xout / 131))
 #  print("gyroskop_yout: ", ("%5d" % gyroskop_yout), " skaliert: ", (gyroskop_yout / 131))
  # print("gyroskop_zout: ", ("%5d" % gyroskop_zout), " skaliert: ", (gyroskop_zout / 131))
   ...
   Blackdata.write("\n"+str(time)+"Gyroscope data(x,y,z)"+"," +str(AngleDegX) +","+str(AngleDegY)+","+str(AngleDegZ) +","+"Accelerometer"+ str(beschleunigung_xout_skaliert)+ ","+ str(beschleunigung_yout_skaliert) +","+ str(beschleunigung_zout_skaliert))
   font = cv2.FONT_HERSHEY_SIMPLEX
   cv2.putText(frame,"Gyroscope data(x,y,z)",(15,15),cv2.FONT_HERSHEY_SIMPLEX,0.7,(0,255,0),1,-1)
   Servoanglecontrol(abs(AngleDegY),abs(AngleDegZ))
   
cap.release()
videoOut.release() #Video Writer  
